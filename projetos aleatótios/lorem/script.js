const text = [
  ...
  const formulario = document.querySelector(".lorem-form");
  ...
  formulario.addEventListener("submit", function(elemento){
    ...
    const aleatorio = Math.floor(Math.random()*text.length);

    if(isNaN(valor) || valor< 0 || valor > 9){
        resultado.innerHTML = `<p class="result">${text[aleatorio]}</P>`
    }else{
        let textoTemporario =text.slice(0,valor);
        textoTemporario = textoTemporario.map(function(item){
            return `<p class="result">${item}</p>`
        }).join("")
        resultado.innerHTML=textoTemporario
    }
  })